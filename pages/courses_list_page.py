...
class CoursesListPage(BasePage):
    def __init__(self, page: Page):
        ...
        #Пустой бллок при отсутствии курсов
    ...
    def check_visible_empty_view(self):
        ...
        expect(self.empty_view_description).to_have_text("Results from the load test pipeline will be desplayed here")
    ...
    def click_edit_course(self, index: int):
        ...
        #После нажатия на кнопку меню обязательно проверяем, что меню открылось и меню айтем виден
        expect(self.course_edit_menu_item_nth(index)).to_be_visible()
        self.course_edit_menu_item.nth(index).click()
        
    def click_delete_course(self, index: int):
        self.course_menu_button.nth(index).click()
        
        expect(self.course_delete_menu_item_nth(index)).to_be_visible()
